 ...
 *
 */
public class DocDeliveryContractCopyTest extends WebdriverBaseCase{
	...
	public void DocDeliveryContractCopy() {
		boolean flag = false;
		beforeTest("DocDeliveryContractCopy");
		try {
			HomePage = new HomePage(webDriver);
			UserInfo = (UserInfo) FrameworkDao.getRandomObjectByParam(
					"UserInfo.getUserByParam", "testLogin");
			CustomerManagerName = UserInfo.getLoginname();
			HomePage = HomePage.goMyTaskPage(0).MyTaskPageStartCredit().goDocDeliveryContractCopyDetailToDo().DocDeliveryContractCopy(CustomerManagerName);
		} catch (Exception e) {
			logger.error("testLogin error：", e);
		}
	}
