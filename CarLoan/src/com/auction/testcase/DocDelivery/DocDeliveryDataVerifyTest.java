 ...
 *
 */
public class DocDeliveryDataVerifyTest extends WebdriverBaseCase{
	...
	public void DocDeliveryDataVerify() {
		boolean flag = false;
		beforeTest("DocDeliveryDataVerify");
		try {
			HomePage = new HomePage(webDriver);
			UserInfo = (UserInfo) FrameworkDao.getRandomObjectByParam(
					"UserInfo.getUserByParam", "testLogin");
			CustomerManagerName = UserInfo.getLoginname();
			HomePage = HomePage.goMyTaskPage(0).MyTaskPageStartCredit().goDocDeliveryDataVerifyDetailToDo().DocDeliveryStart(CustomerManagerName);
		} catch (Exception e) {
			logger.error("testLogin error：", e);
		}
	}
