...
public class LoanStartSeconCarTest extends WebdriverBaseCase {
	...
	private Logger logger = Logger.getLogger(LoanStartSeconCarTest.class);
...
//			FrameworkDao.addcredit(stCredit);
	...
	public void loanstartsecondcar() {
		boolean flag = false;
		beforeTest("loanstartsecondcar");
		try {
			HomePage = new HomePage(webDriver);
			UserInfo = (UserInfo) FrameworkDao.getRandomObjectByParam(
					"UserInfo.getUserByParam", "testLogin");
			CustomerManagerName = UserInfo.getLoginname();
			HomePage.goMyTaskLoanStartDetailToDo().MyTaskPageStartCredit(1,CustomerManagerName,2,carname);
