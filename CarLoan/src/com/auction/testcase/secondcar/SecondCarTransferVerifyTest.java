...
public class SecondCarTransferVerifyTest extends WebdriverBaseCase {
	...
	private Logger logger = Logger.getLogger(SecondCarTransferVerifyTest.class);
	...
	public void SecondCarTransferVerify() {
		...
		try {
			...
			UserInfo = (UserInfo) FrameworkDao.getRandomObjectByParam(
					"UserInfo.getUserByParam", "testLogin");
			CustomerManagerName = UserInfo.getLoginname();
			HomePage.goMyTaskPage(1).MyTaskPageStartCredit().goMyTaskSecondCarTransferVerifyDetailToDo().SecondCarTransferVerifySubmit(CustomerManagerName);
