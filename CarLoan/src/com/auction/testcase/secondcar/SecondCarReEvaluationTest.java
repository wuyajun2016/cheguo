...
public class SecondCarReEvaluationTest extends WebdriverBaseCase {
	...
	private Logger logger = Logger.getLogger(SecondCarReEvaluationTest.class);
	...
	public void SecondCarPreEvaluation() {
		...
		try {
			...
			UserInfo = (UserInfo) FrameworkDao.getRandomObjectByParam(
					"UserInfo.getUserByParam", "testLogin");
			CustomerManagerName = UserInfo.getLoginname();
			HomePage.goMyTaskPage(0).MyTaskPageStartCredit().goMyTaskSecondCarReEvaluationDetailToDo().SecondCarReEvaluationSubmit(CustomerManagerName);
			HomePage.Logincancel();
