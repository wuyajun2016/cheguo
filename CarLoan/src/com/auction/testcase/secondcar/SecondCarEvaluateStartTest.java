...
public class SecondCarEvaluateStartTest extends WebdriverBaseCase {
	private DbUnitUtil DbUnitUtil;
	private UserInfo UserInfo;
	private LoginPage LoginPage;
	private HomePage HomePage;
	private MyTaskSecondCarEvaluateStartDetailToDo mytasksecondcarevaluatestartdetailtodo;
	protected WebdriverBaseApi webDriver;
	StartCredit stCredit;
	private String CustomerManagerName;  //客户经理名字
	private Logger logger = Logger.getLogger(SecondCarEvaluateStartTest.class);
	...
	public void SecondCarEvaluateStart() {
		boolean flag = false;
		beforeTest("SecondCarEvaluateStart");
		try {
			HomePage = new HomePage(webDriver);
			UserInfo = (UserInfo) FrameworkDao.getRandomObjectByParam(
					"UserInfo.getUserByParam", "testLogin");
			CustomerManagerName = UserInfo.getLoginname();
			HomePage.goMyTaskSecondCarEvaluateStartDetailToDo().SecondCarStart(CustomerManagerName);
