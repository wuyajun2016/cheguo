...
public class SecondCarTransferTest extends WebdriverBaseCase {
	...
	private Logger logger = Logger.getLogger(SecondCarTransferTest.class);
	...
	public void SecondCarTransferStart() {
		boolean flag = false;
		beforeTest("SecondCarTransferStart");
		try {
			HomePage = new HomePage(webDriver);
			UserInfo = (UserInfo) FrameworkDao.getRandomObjectByParam(
					"UserInfo.getUserByParam", "testLogin");
			CustomerManagerName = UserInfo.getLoginname();
			HomePage.goMyTaskSecondCarTransferDetailToDo().SecondCarTransferStart(CustomerManagerName);
